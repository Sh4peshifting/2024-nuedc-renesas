...
void user_spi_WriteRead(spi_ctrl_t *const p_api_ctrl,
                        void const *p_src,
                        void *p_dest,
                        ...
                        spi_bit_width_t const bit_width)
{
    ...
    R_SPI_WriteRead(p_api_ctrl, p_src, p_dest, length, bit_width);
...
{
    user_spi_WriteRead(&SPI_CTRL_HANDLER, &byte, &ret, 1, SPI_BIT_WIDTH_8_BITS); // 把byte 写入，并读出一个值，把它存入ret
